...
function gutenberg_fields_middleware_register_scripts() {
	...
	wp_register_script(
		...
		filemtime( GUTENBERG_FIELDS_MIDDLEWARE_PLUGIN_DIR. '/build/middleware'. $min .'.js' )
	);

	wp_register_style( 'gutenberg-fields-middleware', plugins_url( 'build/middleware-editor'. $min .'.css', __FILE__ ) );
